...
type Message = {
  sender: "user" | "agent";  // Explicitly restrict sender type
  text: string;
};
...
const mockChats: Record<number, ChatData> = {
  1: {
    ...
    messages: [{ sender: "user", text: "Is the house still available?" }],
  },
  2: {
    user: "Mark Davis",
    property: "Studio 205",
    messages: [{ sender: "user", text: "Thank you for accepting my request." }],
  },
  3: {
    user: "Sarah Lee",
    property: "Townhouse 12",
    messages: [{ sender: "user", text: "Can I view the house this weekend?" }],
  },
};
...
const ChatInterface = () => {
  ...
  const handleSendMessage = () => {
    ...
    const updatedMessages: Message[] = [
      ...messages,
      { sender: "agent", text: newMessage }, // Ensure 'sender' matches Message type
    ];
  ...
  return (
    <div className="bg-white p-6 shadow-md rounded-lg h-[80vh] flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <Link href="/dashboard/agent/messages" className="flex items-center text-blue-600">
          <IoArrowBack size={24} />
          <span className="ml-2">Back</span>
        </Link>
        <h2 className="text-xl font-bold text-gray-800">
          {chatData.user} – {chatData.property}
        </h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50 p-4 rounded-lg">
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.sender === "agent" ? "justify-end" : "justify-start"} mb-2`}>
            <div className={`px-4 py-2 rounded-lg shadow ${msg.sender === "agent" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-800"}`}>
              {msg.text}
            </div>
          </div>
        ))}
